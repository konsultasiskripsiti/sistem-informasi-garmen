<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Input Stok Opname Raw Material</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Catat hasil hitung fisik bahan baku dan koreksi stok sistem.</p>
            </div>
            <a href="{{ route('stock-opname.raw-materials') }}" class="rounded-xl border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
    ...
    @php
        $materials = $rawMaterials->map(fn ($rawMaterial) => [
            ...
            'quantity' => number_format((float) $rawMaterial->quantity, 2, '.', ''),
        ])->values();

        $initialDetails = collect(old('details', [['raw_material_id' => '', 'physical_quantity' => '']]))
            ->map(fn (array $detail) => [
    ...
    <div
        x-data="{
            ...
            addDetail() {
                this.details.push({ raw_material_id: '', physical_quantity: '' });
            },
            loadAllMaterials() {
                this.details = this.materials.map((material) => ({
            ...
            removeDetail(index) {
                if (this.details.length === 1) {
                    this.details[0] = { raw_material_id: '', physical_quantity: '' };
                    return;
                }
            ...
            materialData(id) {
                return this.materials.find((item) => String(item.id) === String(id));
            },
            materialUnit(id) {
                return this.materialData(id)?.unit || '-';
            },
            systemQuantity(id) {
                return Number(this.materialData(id)?.quantity || 0);
            },
            adjustment(detail) {
                if (!detail.raw_material_id || detail.physical_quantity === '') {
                    return 0;
                }

                return Number(detail.physical_quantity || 0) - this.systemQuantity(detail.raw_material_id);
            },
            formatNumber(value) {
        ...
        class="space-y-6"
    >
        <form method="POST" action="{{ route('stock-opname.raw-materials.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="grid gap-5 md:grid-cols-3">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="notes" name="notes" rows="1" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200" placeholder="Contoh: opname akhir bulan, koreksi gudang, bahan rusak.">{{ old('notes') }}</textarea>
                ...
                <div class="space-y-6 lg:col-span-2 xl:col-span-3">
                    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                        <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 class="text-lg font-semibold text-gray-800 dark:text-white/90">Detail Raw Material</h2>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambahkan bahan baku yang sudah dihitung stok fisiknya.</p>
                            </div>
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                                <button type="button" @click="loadAllMaterials()" class="inline-flex items-center justify-center rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                                ...
                                <div class="rounded-2xl border border-gray-200 p-4 dark:border-gray-700">
                                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                                        <div class="lg:col-span-4">
                                            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Raw Material</label>
                                            <select :name="`details[${index}][raw_material_id]`" x-model="detail.raw_material_id" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                                ...
                                                <template x-for="material in materials" :key="material.id">
                                                    <option :value="material.id" x-text="`${material.code} - ${material.name}`"></option>
                                                </template>
                                            </select>
                                        ...
                                        <div class="lg:col-span-2">
                                            ...
                                            <input type="text" :value="formatNumber(systemQuantity(detail.raw_material_id))" readonly class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                        </div>

                                        <div class="lg:col-span-2">
                                            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Stok Fisik</label>
                                            <input :name="`details[${index}][physical_quantity]`" x-model="detail.physical_quantity" type="number" step="0.01" min="0" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                        </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('stock-opname.raw-materials') }}" class="rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Cancel</a>
