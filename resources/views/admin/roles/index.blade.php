<x-app-layout>
    @php
        $sortUrl = function (string $column) use ($sort, $direction) {
            return route('roles.index', array_merge(request()->query(), [
                'sort' => $column,
    ...
    <div x-data="{
        ...
        toggleAll(event) {
            this.selected = event.target.checked
                ? Array.from(this.$root.querySelectorAll('input[data-bulk-item=\'role\']')).map((input) => input.value)
                : [];
        },
        syncSelection(event) {
        ...
        submitBulkDelete() {
            ...
            if (confirm(`Delete ${this.selected.length} selected role(s)?`)) {
        ...
        <div class="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('roles.index') }}" class="flex w-full max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
                ...
                @if ($search !== '')
                    <a href="{{ route('roles.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                        Reset
                    </a>
                @endif
            </form>
            ...
            <form
                ...
                method="POST"
                action="{{ route('roles.bulk-destroy') }}"
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-base font-semibold text-gray-800 dark:text-white/90">{{ $role->name }}</p>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Guard: {{ $role->guard_name }}</p>
                            <div class="mt-3 flex flex-wrap gap-2">
                                ...
                                @empty
                                    <span class="text-gray-400 text-theme-sm">No permissions</span>
                                @endforelse
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 flex items-center gap-2">
                        <a href="{{ route('roles.show', $role) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                        <a href="{{ route('roles.edit', $role) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
                ...
                <table class="w-full text-left border-collapse">    
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6">
                                <a href="{{ $sortUrl('name') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Role {{ $sortIcon('name') }}</a>
                            </th>
                            <th class="px-5 py-3 text-left sm:px-6">
                    ...
                    <tbody>
                        @forelse ($roles as $role)
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $role->permissions->count() }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('roles.show', $role) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                        ...
                                        <a href="{{ route('roles.edit', $role) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-10 text-center text-gray-400 sm:px-6">
                                    Belum ada role.
                                </td>
                            </tr>
                        @endforelse
