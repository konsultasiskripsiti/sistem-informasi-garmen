<x-app-layout>
    @php
        $sortUrl = function (string $column) use ($sort, $direction) {
            return route('suppliers.index', array_merge(request()->query(), [
                'sort' => $column,
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Suppliers</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar supplier dengan pencarian, pagination, dan aksi massal.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Total: {{ $suppliers->total() }} suppliers
                </div>
                <a href="{{ route('suppliers.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                    Add Supplier
                </a>
            </div>
        </div>
    ...
    <div x-data="{
        ...
        toggleAll(event) {
            this.selected = event.target.checked
                ? Array.from(this.$root.querySelectorAll('input[data-bulk-item=\'supplier\']')).map((input) => input.value)
                : [];
        },
        syncSelection(event) {
        ...
        submitBulkDelete() {
            ...
            if (confirm(`Delete ${this.selected.length} selected supplier(s)?`)) {
        ...
        <div class="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('suppliers.index') }}" class="flex w-full max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative w-full">
                    ...
                    <input
                        ...
                        name="search"
                        value="{{ $search }}"
                        placeholder="Search by code, supplier, PIC, or phone..."
                ...
                <div class="flex gap-3">
                    ...
                    @if ($search !== '')
                        <a href="{{ route('suppliers.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                            Reset
                        </a>
                    @endif
                </div>
            ...
            <form
                ...
                method="POST"
                action="{{ route('suppliers.bulk-destroy') }}"
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-start gap-3">
                            ...
                            >
                            <div>
                                <p class="text-xs font-medium uppercase tracking-wide text-gray-400">{{ $supplier->supplier_code }}</p>
                                <h3 class="mt-1 text-base font-semibold text-gray-800 dark:text-white/90">{{ $supplier->name }}</h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $supplier->person_in_charge }}</p>
                            </div>
                        </div>
                        <span class="{{ $supplier->is_active ? 'bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' }} inline-flex rounded-full px-2.5 py-1 text-xs font-medium">
                            {{ $supplier->is_active ? 'Active' : 'Inactive' }}
                        </span>
                    </div>

                    <div class="mt-4 space-y-2 text-sm text-gray-500 dark:text-gray-400">
                        <p><span class="font-medium text-gray-700 dark:text-gray-300">Phone:</span> {{ $supplier->phone_number }}</p>
                        <p><span class="font-medium text-gray-700 dark:text-gray-300">Address:</span> {{ $supplier->address }}</p>
                    </div>

                    <div class="mt-4 flex items-center gap-2">
                        <a href="{{ route('suppliers.show', $supplier) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                        <a href="{{ route('suppliers.edit', $supplier) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('supplier_code') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Code {{ $sortIcon('supplier_code') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('name') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Supplier {{ $sortIcon('name') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('person_in_charge') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Person In Charge {{ $sortIcon('person_in_charge') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Address</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('phone_number') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Phone {{ $sortIcon('phone_number') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('is_active') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Status {{ $sortIcon('is_active') }}</a></th>
                    ...
                    <tbody>
                        @forelse ($suppliers as $supplier)
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="font-medium text-gray-700 text-theme-sm dark:text-gray-300">{{ $supplier->supplier_code }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $supplier->name }}</span>
                                            <span class="block text-gray-500 text-theme-xs dark:text-gray-400">ID: {{ $supplier->id }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $supplier->person_in_charge }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="max-w-[200px] truncate text-gray-500 text-theme-sm dark:text-gray-400" title="{{ $supplier->address }}"> {{ $supplier->address ?: '-' }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $supplier->phone_number }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <span class="{{ $supplier->is_active ? 'bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300' }} inline-flex rounded-full px-2.5 py-1 text-xs font-medium">
                                        {{ $supplier->is_active ? 'Active' : 'Inactive' }}
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('suppliers.show', $supplier) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                        ...
                                        <a href="{{ route('suppliers.edit', $supplier) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-5 py-10 text-center text-gray-400 sm:px-6">Belum ada supplier.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
