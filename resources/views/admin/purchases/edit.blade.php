<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Edit Purchase</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui transaksi pembelian bahan baku.</p>
            </div>
    ...
    @php
        ...
        $existingDetails = $purchase->items->map(fn ($item) => [
            'raw_material_id' => (string) $item->raw_material_id,
            'quantity' => $item->quantity,
            'unit_price' => (string) $item->unit_price,
        ])->values()->all();
        $initialDetails = collect(old('details', $existingDetails !== [] ? $existingDetails : [['raw_material_id' => '', 'quantity' => '', 'unit_price' => '']]))
            ->map(fn (array $detail) => [
        ...
        <form method="POST" action="{{ route('purchases.update', $purchase) }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-5">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui data utama transaksi pembelian bahan baku.</p>
                </div>
                ...
                <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                    <div>
                        ...
                        <input id="purchase_date" name="purchase_date" type="date" value="{{ old('purchase_date', $purchase->purchase_date->toDateString()) }}" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                    ...
                    <div>
                        ...
                        <select id="supplier_id" name="supplier_id" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                            ...
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" {{ (string) old('supplier_id', $purchase->supplier_id) === (string) $supplier->id ? 'selected' : '' }}>{{ $supplier->supplier_code }} - {{ $supplier->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="person_in_charge_id" name="person_in_charge_id" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ (string) old('person_in_charge_id', $purchase->person_in_charge_id) === (string) $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2 xl:col-span-1">
                        ...
                        <textarea id="notes" name="notes" rows="1" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">{{ old('notes', $purchase->notes) }}</textarea>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                        <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui satu atau lebih bahan baku dalam transaksi ini.</p>
                            </div>
                                ...
                                <div class="rounded-2xl border border-gray-200 p-4 dark:border-gray-700">
                                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-12 items-start">
                                        ...
                                        <div class="lg:col-span-4">
                                            ...
                                            <select :name="`details[${index}][raw_material_id]`" x-model="detail.raw_material_id" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                                                ...
                                                <template x-for="material in materials" :key="material.id">
                                                    <option :value="material.id" :selected="String(detail.raw_material_id) === String(material.id)" x-text="material.name"></option>
                                                </template>
                                            </select>
                                        ...
                                        <div class="lg:col-span-2">
                                            ...
                                            </div>                                            <p class="mt-2 hidden text-xs lg:block">&nbsp;</p>
                                        </div>
