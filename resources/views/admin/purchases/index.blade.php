<x-app-layout>
    @php
        $sortUrl = function (string $column) use ($sort, $direction) {
            return route('purchases.index', array_merge(request()->query(), [
                'sort' => $column,
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Pembelian Bahan Baku</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar transaksi pembelian bahan baku beserta total harganya.</p>
            </div>
            <a href="{{ route('purchases.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                Add Purchase
            </a>
        </div>
    </x-slot>
    ...
    <div class="mb-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <form method="GET" action="{{ route('purchases.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="relative w-full max-w-xl">
                ...
                <input
                    ...
                    name="search"
                    value="{{ $search }}"
                    placeholder="Search supplier, PIC, atau keterangan..."
            ...
            <div class="flex gap-3">
                ...
                @if ($search !== '')
                    <a href="{{ route('purchases.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                        Reset
                    </a>
                @endif
            </div>
    ...
    <div class="grid gap-4 lg:hidden">
        @forelse ($purchases as $purchase)
            <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-xs font-medium uppercase tracking-wide text-gray-400">{{ $purchase->purchase_date->format('d M Y') }}</p>
                <h3 class="mt-1 text-base font-semibold text-gray-800 dark:text-white/90">{{ $purchase->supplier->name }}</h3>
                <div class="mt-3 space-y-2 text-sm text-gray-500 dark:text-gray-400">
                    <p><span class="font-medium text-gray-700 dark:text-gray-300">PIC:</span> {{ $purchase->personInCharge->name }}</p>
                    <p><span class="font-medium text-gray-700 dark:text-gray-300">Items:</span> {{ $purchase->items->count() }} bahan</p>
                    <p><span class="font-medium text-gray-700 dark:text-gray-300">Total:</span> Rp{{ number_format($purchase->total_amount, 0, ',', '.') }}</p>
                </div>

                <div class="mt-4 flex items-center gap-2">
                    <a href="{{ route('purchases.show', $purchase) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                    <a href="{{ route('purchases.edit', $purchase) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
            ...
            <table class="w-full min-w-[980px]">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('purchase_date') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Tanggal {{ $sortIcon('purchase_date') }}</a></th>
                        <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Supplier</p></th>
                        <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">PIC</p></th>
                ...
                <tbody>
                    @forelse ($purchases as $purchase)
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $purchase->purchase_date->format('d M Y') }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $purchase->supplier->name }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $purchase->personInCharge->name }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @if ($purchase->items->count() > 3)
                                        <span class="inline-flex rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                            +{{ $purchase->items->count() - 3 }} lagi
                                        </span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-700 text-theme-sm dark:text-gray-300">Rp{{ number_format($purchase->total_amount, 0, ',', '.') }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('purchases.show', $purchase) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                    ...
                                    <a href="{{ route('purchases.edit', $purchase) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-gray-400 sm:px-6">Belum ada transaksi pembelian.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
