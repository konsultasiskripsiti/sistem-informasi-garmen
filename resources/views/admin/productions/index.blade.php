<x-app-layout>
    @php
        $sortUrl = function (string $column) use ($sort, $direction) {
            return route('productions.index', array_merge(request()->query(), [
                'sort' => $column,
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Produksi Product</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar transaksi produksi product dari template bahan baku yang sudah disiapkan.</p>
            </div>
            <a href="{{ route('productions.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                Add Production
            </a>
        </div>
    </x-slot>
    ...
    <div class="mb-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
        <form method="GET" action="{{ route('productions.index') }}" class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="relative w-full max-w-xl">
                ...
                <input
                    ...
                    name="search"
                    value="{{ $search }}"
                    placeholder="Search product atau keterangan..."
            ...
            <div class="flex gap-3">
                ...
                @if ($search !== '')
                    <a href="{{ route('productions.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                        Reset
                    </a>
                @endif
            </div>
    ...
    <div class="grid gap-4 lg:hidden">
        @forelse ($productions as $production)
            <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-xs font-medium uppercase tracking-wide text-gray-400">{{ $production->production_date->format('d M Y') }}</p>
                <h3 class="mt-1 text-base font-semibold text-gray-800 dark:text-white/90">{{ $production->product->name }}</h3>
                <div class="mt-3 space-y-2 text-sm text-gray-500 dark:text-gray-400">
                    <p><span class="font-medium text-gray-700 dark:text-gray-300">Jumlah Produksi:</span> {{ number_format($production->production_quantity) }} {{ $production->product->unit }}</p>
                    <p><span class="font-medium text-gray-700 dark:text-gray-300">Detail Bahan:</span> {{ $production->items->count() }} item</p>
                </div>
                <div class="mt-4 flex items-center gap-2">
                    <a href="{{ route('productions.show', $production) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                    <a href="{{ route('productions.edit', $production) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
            ...
            <table class="w-full min-w-[900px]">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('production_date') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Tanggal {{ $sortIcon('production_date') }}</a></th>
                        <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Product</p></th>
                        <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('production_quantity') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Jumlah {{ $sortIcon('production_quantity') }}</a></th>
                ...
                <tbody>
                    @forelse ($productions as $production)
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $production->production_date->format('d M Y') }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $production->product->name }}</p>
                                <p class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $production->product->size }} / {{ $production->product->unit }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ number_format($production->production_quantity) }}</p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($production->items->take(3) as $item)
                                    ...
                                    @endforeach
                                    @if ($production->items->count() > 3)
                                        <span class="inline-flex rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                            +{{ $production->items->count() - 3 }} lagi
                                        </span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('productions.show', $production) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                    ...
                                    <a href="{{ route('productions.edit', $production) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-10 text-center text-gray-400 sm:px-6">Belum ada transaksi produksi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="mt-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-4 dark:border-gray-800 dark:bg-white/[0.03] md:flex-row md:items-center md:justify-between">
        <div class="text-sm text-gray-500 dark:text-gray-400">
            Showing {{ $productions->firstItem() ?? 0 }} to {{ $productions->lastItem() ?? 0 }} of {{ $productions->total() }} productions
        </div>
        <div>
