<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Create Production</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Tambahkan transaksi produksi product dari template bahan baku.</p>
            </div>
    ...
    <div
        x-data="{
            ...
            productionQuantity: '{{ old('production_quantity', 1) }}',
            selectedProduct() {
        ...
        <form method="POST" action="{{ route('productions.store') }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-5">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Isi informasi utama transaksi produksi product.</p>
                </div>
                ...
                <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                    <div>
                        ...
                        <input id="production_date" name="production_date" type="date" value="{{ old('production_date', now()->toDateString()) }}" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                    ...
                    <div>
                        ...
                        <select id="product_id" name="product_id" x-model="productId" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" {{ (string) old('product_id') === (string) $product->id ? 'selected' : '' }}>{{ $product->name }} - {{ $product->size }}</option>
                            @endforeach
                        </select>
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">Create Production</button>
            </div>
        </form>
    </div>
</x-app-layout>
