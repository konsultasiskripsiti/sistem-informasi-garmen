    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar user dengan pencarian, pagination, dan aksi massal.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Total: {{ $users->total() }} users
                </div>
                <a href="{{ route('users.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                    Add User
                </a>
            </div>
        </div>
    ...
    <div x-data="{
        ...
        toggleAll(event) {
            this.selected = event.target.checked
                ? Array.from(this.$root.querySelectorAll('input[data-bulk-item=\'user\']')).map((input) => input.value)
                : [];
        },
        syncSelection(event) {
        ...
        submitBulkDelete() {
            ...
            if (confirm(`Delete ${this.selected.length} selected user(s)?`)) {
        ...
        <div class="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('users.index') }}" class="flex w-full max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
                ...
                @if ($search !== '')
                    <a href="{{ route('users.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                        Reset
                    </a>
                @endif
            </form>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-4">
                                <p class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-400">Roles</p>
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @empty
                                        <span class="text-gray-400 text-theme-sm">No roles</span>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-4 flex items-center gap-2">
                        <a href="{{ route('users.show', $user) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                        <a href="{{ route('users.edit', $user) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
                ...
                <table class="w-full text-left border-collapse">    
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6">
                                <a href="{{ $sortUrl('name') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">User {{ $sortIcon('name') }}</a>
                            </th>
                            <th class="px-5 py-3 text-left sm:px-6">
                                <a href="{{ $sortUrl('email') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Email {{ $sortIcon('email') }}</a>
                            </th>
                            <th class="px-5 py-3 text-left sm:px-6">
                    ...
                    <tbody>
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('users.show', $user) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                        ...
                                        <a href="{{ route('users.edit', $user) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-gray-400 sm:px-6">
                                    Belum ada user.
                                </td>
                            </tr>
                        @endforelse
