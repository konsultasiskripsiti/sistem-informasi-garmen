<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui product dan detail bahan bakunya.</p>
            </div>
    ...
    @php
        ...
        $existingDetails = $product->rawMaterials->map(fn ($rawMaterial) => [
            'raw_material_id' => (string) $rawMaterial->id,
            'quantity' => $rawMaterial->pivot->quantity,
        ])->values()->all();
        $initialDetails = collect(old('details', $existingDetails !== [] ? $existingDetails : [['raw_material_id' => '', 'quantity' => '']]))
            ->map(fn (array $detail) => [
    ...
    <div
        x-data="{
            ...
            displayPrice: '',
            init() {
                this.formatVisiblePrice('{{ old('unit_price', $product->unit_price) }}');
            },
            addDetail() {
        ...
        <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-6">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-5">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui data utama product dan harga satuan.</p>
                </div>

                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        <label for="name" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Product</label>
                        <input id="name" name="name" type="text" value="{{ old('name', $product->name) }}" class="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 outline-none transition focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">
                ...
                <div class="mt-5 grid gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        >
                        <input type="hidden" name="unit_price" x-ref="unitPrice" value="{{ old('unit_price', $product->unit_price) }}">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="mb-5 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Atur bahan baku yang digunakan oleh product ini.</p>
                    </div>
