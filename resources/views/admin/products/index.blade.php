    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar product dengan detail bahan baku.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Total: {{ $products->total() }} products
                </div>
                <a href="{{ route('products.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                    Add Product
                </a>
            </div>
        </div>
    ...
    <div x-data="{
        ...
        toggleAll(event) {
            this.selected = event.target.checked
                ? Array.from(this.$root.querySelectorAll('input[data-bulk-item=\'product\']')).map((input) => input.value)
                : [];
        },
        syncSelection(event) {
        ...
        submitBulkDelete() {
            ...
            if (confirm(`Delete ${this.selected.length} selected product(s)?`)) {
        ...
        <div class="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('products.index') }}" class="flex w-full max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative w-full">
                    ...
                    <input
                        ...
                        name="search"
                        value="{{ $search }}"
                        placeholder="Search by product, size, or unit..."
                ...
                <div class="flex gap-3">
                    ...
                    @if ($search !== '')
                        <a href="{{ route('products.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                            Reset
                        </a>
                    @endif
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Bahan baku: {{ $product->rawMaterials->count() }} item</p>
                        </div>
                    </div>

                    <div class="mt-4 flex items-center gap-2">
                        <a href="{{ route('products.show', $product) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                        <a href="{{ route('products.edit', $product) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
                            ...
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div>
                                        ...
                                        <span class="block text-gray-500 text-theme-xs dark:text-gray-400">ID: {{ $product->id }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <span class="inline-flex rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">{{ $product->size }}</span>
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="font-medium text-gray-700 text-theme-sm dark:text-gray-300">Rp{{ number_format($product->unit_price, 0, ',', '.') }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($product->rawMaterials->take(3) as $rawMaterial)
                                        ...
                                        @endforeach
                                        @if ($product->rawMaterials->count() > 3)
                                            <span class="inline-flex rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                                +{{ $product->rawMaterials->count() - 3 }} lagi
                                            </span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('products.show', $product) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                        ...
                                        <a href="{{ route('products.edit', $product) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
