<x-app-layout>
    @php
        $sortUrl = function (string $column) use ($sort, $direction) {
            return route('raw-materials.index', array_merge(request()->query(), [
                'sort' => $column,
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Raw Materials</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar bahan baku dengan pencarian, pagination, dan aksi massal.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Total: {{ $rawMaterials->total() }} raw materials
                </div>
                <a href="{{ route('raw-materials.create') }}" class="rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
                    Add Raw Material
                </a>
            </div>
        </div>
    ...
    <div x-data="{
        ...
        toggleAll(event) {
            this.selected = event.target.checked
                ? Array.from(this.$root.querySelectorAll('input[data-bulk-item=\'raw-material\']')).map((input) => input.value)
                : [];
        },
        syncSelection(event) {
        ...
        submitBulkDelete() {
            ...
            if (confirm(`Delete ${this.selected.length} selected raw material(s)?`)) {
        ...
        <div class="mb-4 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] lg:flex-row lg:items-center lg:justify-between">
            <form method="GET" action="{{ route('raw-materials.index') }}" class="flex w-full max-w-xl flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div class="flex gap-3">
                    ...
                    @if ($search !== '')
                        <a href="{{ route('raw-materials.index') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium text-gray-700 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">
                            Reset
                        </a>
                    @endif
                </div>
            ...
            <form
                ...
                method="POST"
                action="{{ route('raw-materials.bulk-destroy') }}"
            ...
            >
                ...
                <template x-for="id in selected" :key="id">
                    <input type="hidden" name="raw_material_ids[]" :value="id">
                </template>
            ...
            @forelse ($rawMaterials as $rawMaterial)
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start gap-3">
                        <input
                            type="checkbox"
                            data-bulk-item="raw-material"
                            value="{{ $rawMaterial->id }}"
                            @change="syncSelection($event)"
                            :checked="selected.includes('{{ $rawMaterial->id }}')"
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-3 space-y-2 text-sm text-gray-500 dark:text-gray-400">
                                <p><span class="font-medium text-gray-700 dark:text-gray-300">Jumlah:</span> {{ number_format((float) $rawMaterial->quantity, 2) }}</p>
                                <p><span class="font-medium text-gray-700 dark:text-gray-300">Satuan:</span> {{ $rawMaterial->unit }}</p>
                                <p><span class="font-medium text-gray-700 dark:text-gray-300">Keterangan:</span> {{ $rawMaterial->description ?: '-' }}</p>
                            </div>
                        </div>
                    </div>

                    <div class="mt-4 flex items-center gap-2">
                        <a href="{{ route('raw-materials.show', $rawMaterial) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">View</a>
                        <a href="{{ route('raw-materials.edit', $rawMaterial) }}" class="inline-flex items-center rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300">Edit</a>
                        <form method="POST" action="{{ route('raw-materials.destroy', $rawMaterial) }}" onsubmit="return confirm('Delete this raw material?');">
                            @csrf
                            @method('DELETE')
        ...
        <div class="hidden lg:block overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <table class="w-full text-left border-collapse">    
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('raw_material_code') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Code {{ $sortIcon('raw_material_code') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('name') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Raw Material {{ $sortIcon('name') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('quantity') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Quantity {{ $sortIcon('quantity') }}</a></th>
                            <th class="px-5 py-3 text-left sm:px-6"><a href="{{ $sortUrl('unit') }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Unit {{ $sortIcon('unit') }}</a></th>
                    ...
                    <tbody>
                        @forelse ($rawMaterials as $rawMaterial)
                            <tr class="border-b border-gray-100 dark:border-gray-800">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <div>
                                        <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $rawMaterial->name }}</span>
                                        <span class="block text-gray-500 text-theme-xs dark:text-gray-400">ID: {{ $rawMaterial->id }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ number_format((float) $rawMaterial->quantity, 2) }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <span class="inline-flex rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">{{ $rawMaterial->unit }}</span>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="max-w-[200px] truncate text-gray-500 text-theme-sm dark:text-gray-400" title="{{ $rawMaterial->description }}"> {{ $rawMaterial->description ?: '-' }} </p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('raw-materials.show', $rawMaterial) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="View">
                                        ...
                                        <a href="{{ route('raw-materials.edit', $rawMaterial) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-brand-300 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300" title="Edit">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-10 text-center text-gray-400 sm:px-6">Belum ada raw material.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
